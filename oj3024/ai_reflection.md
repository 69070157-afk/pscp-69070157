...
| OJ submission ID, if submitted | 566114	
 |
| OJ status | Pass |
...
```text
ผมถามว่า ตัวโค้ดผมมีข้อบกพร่องตรงไหนทําไมถึงไม่ผ่านเพราะตอนนั้นผมไม่ผ่านอยู่ 2 test case 
```
...
```text
 ผมกลับไปอ่านโจทย์ใหม่แล้วเทียบกับตัวโคด แล้วพบว่าผมเข้าใจผิดว่า ถ้า >= 2 Surprising ซึงความจริงควรจะไม่ เพราะเค้าบอกว่าถ้าห่างกันเกินสองคะแนนต้อง Surprising ซึ่ง 2 ห่างไม่เกิน 2 จึง Not Surprising
```
...
```text
ผมต้องอ่านโจทย์ให้ละเอียดและต้องตีโจทย์ให้แตกก่อนเริ่มเขียน เพราะ code ไม่เคยผิดมีแค่คนที่ผิด
```
